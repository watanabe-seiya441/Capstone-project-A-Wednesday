...
import {colors, en_jp, unit} from './config.js'
    ...
    svg.selectAll("path")
        ...
        .attr("fill", (d) => {
            ...
            if (value) { // 値が存在する場合
                ...
                // return `rgb(${color_given_range(numericValue,255, 245)},${color_given_range(numericValue,255, 113)},${color_given_range(numericValue,255, 45)})`    
...
const Button_and_Bar = (csvData ,topojsonData, info, donationPositionData, newsData) => {
    ...
    selectVariable.style.width = "300px;"; // 幅を設定
    selectVariable.style.height = "30px"; // 高さを設定
    selectVariable.style.fontSize = "15px"; // フォントサイズを設定
    selectVariable.style.margin = "20px"; // マージンを設定
    ...
    slider.style.left = "460px";
    slider.style.bottom = "20px"; // ページの最下部から
    slider.value = year;
